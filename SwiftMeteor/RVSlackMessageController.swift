//
//  RVSlackMessageController.swift
//  SwiftMeteor
//
...
class RVSlackMessageController: SLKTextViewController {
    ...
    func commonInit() {
        ...
        NotificationCenter.default.addObserver(self,  selector: #selector(RVSlackMessageController.textInputbarDidMove(_:)), name: NSNotification.Name.SLKTextInputbarDidMove, object: nil)
    }
    
    override func viewDidLoad() {
        
        // Register a SLKTextView subclass, if you need any special appearance and/or behavior customisation.
   //     self.registerClass(forTextView: RVSlackMessageView.classForCoder())
   ...
   //         self.registerClass(forTypingIndicatorView: RVSlackTypingIndicatorView.classForCoder())
        ...
        self.tableView.register(RVSlackMessageViewCell.classForCoder(), forCellReuseIdentifier: RVSlackMessageViewCell.identifier)
        
        self.autoCompletionView.register(RVSlackMessageViewCell.classForCoder(), forCellReuseIdentifier: RVSlackMessageViewCell.autoCompletionCellIdentifier)
...
extension RVSlackMessageController {
    ...
    func configureActionItems() {
        ...
        let arrowItem = UIBarButtonItem(image: UIImage(named: "icn_arrow_down"), style: .plain, target: self, action: #selector(RVSlackMessageController.hideOrShowTextInputbar(_:)))
        let editItem = UIBarButtonItem(image: UIImage(named: "icn_editing"), style: .plain, target: self, action: #selector(RVSlackMessageController.editRandomMessage(_:)))
        let typeItem = UIBarButtonItem(image: UIImage(named: "icn_typing"), style: .plain, target: self, action: #selector(RVSlackMessageController.simulateUserTyping(_:)))
        let appendItem = UIBarButtonItem(image: UIImage(named: "icn_append"), style: .plain, target: self, action: #selector(RVSlackMessageController.fillWithText(_:)))
        let pipItem = UIBarButtonItem(image: UIImage(named: "icn_pic"), style: .plain, target: self, action: #selector(RVSlackMessageController.togglePIPWindow(_:)))
    ...
    func simulateUserTyping(_ sender: AnyObject) {
        ...
        if DEBUG_CUSTOM_TYPING_INDICATOR == true {
            ...
            let imgSize = CGSize(width: RVSlackTypingIndicatorView.typingIndicatorViewHeight.minimum.rawValue*scale, height: RVSlackTypingIndicatorView.typingIndicatorViewHeight.avator.rawValue*scale)
    ...
    func editCellMessage(_ gesture: UIGestureRecognizer) {
        ...
        guard let cell = gesture.view as? RVSlackMessageViewCell else {
            return
        }
        ...
        self.tableView.scrollToRow(at: (cell.indexPath ) as IndexPath, at: .bottom, animated: true)
    }
...
extension RVSlackMessageController {
    ...
    func messageCellForRowAtIndexPath(_ indexPath: IndexPath) -> RVSlackMessageViewCell {
        
        let cell = self.tableView.dequeueReusableCell(withIdentifier: RVSlackMessageViewCell.identifier) as! RVSlackMessageViewCell
        
        if cell.gestureRecognizers?.count == nil {
            let longPress = UILongPressGestureRecognizer(target: self, action: #selector(RVSlackMessageController.didLongPressCell(_:)))
    ...
    func autoCompletionCellForRowAtIndexPath(_ indexPath: IndexPath) -> RVSlackMessageViewCell {
        
        let cell = self.autoCompletionView.dequeueReusableCell(withIdentifier: RVSlackMessageViewCell.autoCompletionCellIdentifier) as! RVSlackMessageViewCell
        cell.indexPath = indexPath as NSIndexPath
    ...
    override func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        if tableView == self.tableView {
            ...
            var width = tableView.frame.width-RVSlackMessageViewCell.avatarHeight
            width -= 25.0
            ...
            if height < RVSlackMessageViewCell.minimumHeight {
                height = RVSlackMessageViewCell.minimumHeight
            }
            
            return height
        }
        else {
            return RVSlackMessageViewCell.minimumHeight
        }
    }
