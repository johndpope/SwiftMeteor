...
import Foundation
import UIKit
class RVUserListConfiguration8: RVBaseConfiguration8 {
    ...
    override var mainDatasource: RVBaseDatasource4<RVBaseModel> {
        let datasource = RVUserListDatasource8<RVBaseModel>(manager: self.manager, datasourceType: .main, maxSize: 80)
      //  datasource.subscription = self.subscription
        return datasource
    }
    override var filterDatasource: RVBaseDatasource4<RVBaseModel> {
        return RVUserListDatasource8<RVBaseModel>(manager: self.manager, datasourceType: .filter, maxSize: self.mainDatasourceMaxSize)
    }
