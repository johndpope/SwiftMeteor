...
class RVBaseSLKViewController4: SLKTextViewController {
    ...
    var instanceConfiguration: RVListControllerConfigurationProtocol { return RVBaseConfiguration4<RVSubbaseModel>(scrollView: dsScrollView) }
...
extension RVBaseSLKViewController4: RVFirstViewHeaderCellDelegate {
    func expandCollapseButtonTouched(view: RVFirstViewHeaderCell) {
        if let datasource = view.datasource4 as? RVBaseDatasource4<RVSubbaseModel> {
            ...
            configuration.toggle(datasource: datasource, callback: { error  in
                if let error = error {
...
extension RVBaseSLKViewController4 {
   ...
   //         let nib = Bundle.main.loadNibNamed(RVTransactionTableViewCell.identifier, owner: nil, options: nil)
   //         let nib = UINib(nibName: RVTransactionTableViewCell.identifier, bundle: nil)
    //        tableView.register(nib, forCellReuseIdentifier: RVTransactionTableViewCell.identifier)
            tableView.register(RVTransactionTableViewCell.self, forCellReuseIdentifier: RVTransactionTableViewCell.identifier)
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(RVBaseSLKViewController4.stateDidChange(_:)), name: RVNotification.AppStateChanged, object: nil)
    }
